...
CREATE OR REFRESH MATERIALIZED VIEW churn_app_events
COMMENT "Application events and sessions"
AS SELECT *  FROM read_files(
  '/Volumes/main/fukiat_julnual_databricks_com_retail/retail/events',
  format => 'csv',
  header => true,
  mode => 'FAILFAST')
...
AS SELECT *  FROM read_files(
  '/Volumes/main/fukiat_julnual_databricks_com_retail/retail/orders',
  format => 'json',
  header => true,
  mode => 'FAILFAST')
...
-- CREATE  LIVE TABLE churn_users_bronze (
...
-- AS SELECT * FROM cloud_files("/Volumes/main/fukiat_julnual_databricks_com_retail/retail/users", "json", map("cloudFiles.inferColumnTypes", "true"))

CREATE OR REFRESH MATERIALIZED VIEW churn_users_bronze 
COMMENT "raw user data coming from json files ingested in incremental with Auto Loader to support schema inference and evolution"
AS SELECT *  FROM read_files(
  '/Volumes/main/fukiat_julnual_databricks_com_retail/retail/users',
  format => 'json',
  header => true,
  mode => 'FAILFAST')
...
from live.churn_users_bronze

-- COMMAND ----------

-- DBTITLE 1,Clean orders
CREATE LIVE TABLE churn_orders (
